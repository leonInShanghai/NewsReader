...
public class MainActivity extends Activity {
    ...
    private void requestData(){
        ...
        //持久化保存请求的时间
        SpUtils.setLong(getApplicationContext(),CACHE_ADS_TIME,timeMillis + 86400);

        String[] leAdsArr = Constant.LE_ADS_ARR;

        //生成[0,leAdsArr.length - 1)区间的整数：
        Random r = new Random();
        int n = r.nextInt(leAdsArr.length - 1);

        //获取到一张随机的启动图URL
        String launchStr = leAdsArr[n];

        ImageBigPlaceholderUtil.getSinstance().displayPic(launchStr,mIv_ad);
    }
...
// 网易 启动页 广告 2019-4-6 停止服务自己造数据 大改前 备份代码
